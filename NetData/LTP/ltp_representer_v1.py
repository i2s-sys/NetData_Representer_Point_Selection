...
class RouteLTPMatrix(nn.Module):
    """
    Route/time adaptation of NetData.models.LTP.

    The original LTP model uses user/item/time embeddings. For the matrix
    completion setting here we only keep:
    - route embedding
    - time-sequence embedding
    """

    def __init__(self, num_routes, num_time, embedding_dim, period):
        super().__init__()
        ...
        self.time_embeddings = nn.Parameter(
            nn.init.xavier_uniform_(torch.empty(num_time + period, embedding_dim))
        )

        self.lstm = nn.LSTM(embedding_dim, embedding_dim, 1, batch_first=True)
        self.attention = Attention(embedding_dim)

        self.route_projection = nn.Parameter(torch.eye(embedding_dim))
        self.route_bias = nn.Parameter(torch.zeros(embedding_dim))
        self.time_projection = nn.Parameter(torch.eye(embedding_dim))
        self.time_bias = nn.Parameter(torch.zeros(embedding_dim))

    def forward(self, route_idx, time_idx, seq_idx):
        del time_idx
        ...
        seq_embeds = self.time_embeddings[seq_idx]
        seq_embeds, _ = self.lstm(seq_embeds)
        time_context = self.attention(seq_embeds)

        route_embeds = torch.matmul(route_embeds, self.route_projection) + self.route_bias
        time_context = torch.matmul(time_context, self.time_projection) + self.time_bias

        return torch.sum(route_embeds * time_context, dim=1)
...
def build_time_sequences(time_indices, period):
    time_indices = np.asarray(time_indices, dtype=np.int64).reshape(-1)
    if period <= 0:
        raise ValueError("period must be positive")
    seq = time_indices[:, None] + np.arange(-period, 0, dtype=np.int64)[None, :]
...
def compute_sample_importance_gradient(model, route_idx, time_idx, seq_idx, criterion, values, batch_size=64):
    route_grad_norms = []
    time_grad_norms = []
    sample_importances = []
    sample_grads = []
    ...
    # cuDNN-backed LSTM requires training mode for backward().
    # This model has no dropout / batchnorm, so using train mode here is safe.
    model.train()
...
def compute_metrics(predictions, ground_truth):
    ...
    mape = float(np.mean(np.abs(valid_predictions - valid_ground_truth) / np.maximum(np.abs(valid_ground_truth), 1e-8)))
...
class OnlineLTPRepresenterLearner:
    def __init__(self, matrix_data, config):
        ...
        self.period = int(config.get("period", 5))
        ...
        self.save_dir = config.get("save_dir", "./online_results_ltp")
        ...
        self.train_seq_indices_tensor = None
    ...
    def _samples_to_tensors(self, samples):
        ...
        seq_indices = build_time_sequences(time_indices, self.period)
        ...
        return route_tensor, time_tensor, seq_tensor, value_tensor
    ...
    def _train_current_model(self, route_indices, time_indices, seq_indices, values, epochs, verbose):
        if values is None:
            return None
        ...
        for epoch in range(epochs):
            ...
            for batch_routes, batch_times, batch_seq, batch_values in train_loader:
                self.optimizer.zero_grad(set_to_none=True)
                predictions = self.model(batch_routes, batch_times, batch_seq)
            ...
            self.model.eval()
            with torch.no_grad():
                val_predictions = self.model(val_route_indices, val_time_indices, val_seq_indices)
        ...
        return {
            ...
            "train_seq_indices": train_seq_indices,
    ...
    def train_model_with_representer(self, epochs=None, verbose=True):
        ...
        route_indices, time_indices, seq_indices, values = self.training_data
        if values is None:
            return None

        artifacts = self._train_current_model(route_indices, time_indices, seq_indices, values, epochs, verbose)
        ...
        self.train_seq_indices_tensor = artifacts["train_seq_indices"]
    ...
    def select_mixed_routes(self, route_importances, from_top_routes=False):
        ...
        if from_top_routes:
            important_pool_size = min(
                self.num_routes,
                max(num_top_routes + num_other_routes, int(round(self.num_routes * min(self.route_selection_ratio * 2.0, 1.0)))),
            )
        ...
        return selected_routes.astype(np.int64), np.asarray(top_routes, dtype=np.int64), np.asarray(other_routes, dtype=np.int64)
    ...
    def train_model_with_selected_routes(self, selected_route_indices, epochs=None, verbose=True, seed_offset=0):
        ...
        route_indices, time_indices, seq_indices, values = data
        if values is None:
            return None
        return self._train_current_model(route_indices, time_indices, seq_indices, values, epochs, verbose)
    ...
    def predict_routes(self, route_indices, target_time_idx, batch_size=4096):
        ...
        seq_indices = build_time_sequences(target_times, self.period)
        ...
        with torch.no_grad():
            for start in range(0, len(route_indices), batch_size):
                ...
                seq_tensor = torch.tensor(seq_indices[start:end], dtype=torch.long, device=device)
                batch_predictions = self.model(route_tensor, time_tensor, seq_tensor)
    ...
    def experiment_with_routes(self, exp_key, route_indices, exp_name, use_stage1_init=True, seed_offset=0):
        ...
        if metrics is None:
            print("  no valid ground-truth values at target time, predictions were still generated")
            metrics = {}
        else:
            print(
                "  metrics | "
    ...
    def run_online_learning(self):
        print("=" * 80)
        print("LTP representer matrix completion")
        print("=" * 80)
...
def build_target_times(args, num_time):
    if args.target_time is not None:
        ...
        start = args.target_start if args.target_start is not None else args.period
...
def create_argument_parser():
    parser = argparse.ArgumentParser(
        description="LTP-based representer point route selection and matrix completion"
    )
    ...
    parser.add_argument("--route-selection-ratio", type=float, default=0.25)
...
def main():
    ...
    if args.output_dir is None:
        output_dir = f"./Result_LTP_{matrix_filename}_{int(args.route_selection_ratio * 100)}"
    else:
    ...
    for index, target_time in enumerate(target_times, start=1):
        ...
        learner = OnlineLTPRepresenterLearner(matrix_data, config)
