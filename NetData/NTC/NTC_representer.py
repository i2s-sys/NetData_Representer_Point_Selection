...
class RouteNTCMatrix(nn.Module):
    """
    Route/time adaptation of NetData.models.NTC.

    The original NTC builds a 3D interaction tensor from user/item/time
    embeddings and applies 3D convolutions. For matrix completion we only keep:
    - route embedding
    - time embedding

    Their outer product forms a 2D interaction map, then a 2D CNN predicts
    the value at the target time.
    """

    def __init__(self, num_routes, num_time, embedding_dim, channels):
        super().__init__()
        if embedding_dim < 8:
            raise ValueError("embedding_dim must be at least 8 for the NTC conv stack")
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(1, channels, kernel_size=2, stride=2),
        ...
        with torch.no_grad():
            dummy = torch.zeros(1, 1, embedding_dim, embedding_dim)
            conv_out = self.conv(dummy)
            flat_dim = conv_out.reshape(1, -1).size(1)
...
class OnlineNTCRepresenterLearner:
    def __init__(self, matrix_data, config):
        ...
        self.channels = int(config.get("channels", 32))
        ...
        self.save_dir = config.get("save_dir", "./online_results_ntc")
    ...
    def run_online_learning(self):
        print("=" * 80)
        print("NTC representer matrix completion")
        print("=" * 80)
...
def create_argument_parser():
    parser = argparse.ArgumentParser(
        description="NTC-based representer point route selection and matrix completion"
    )
    ...
    parser.add_argument("--route-selection-ratio", type=float, default=0.3)
...
def main():
    ...
    if args.output_dir is None:
        output_dir = f"./Result_NTC_{matrix_filename}_{int(args.route_selection_ratio * 100)}"
    else:
    ...
    for index, target_time in enumerate(target_times, start=1):
        ...
        learner = OnlineNTCRepresenterLearner(matrix_data, config)
